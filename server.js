...
function parseFromPlainText(text, iso) {
  // Heuristic parser from the visible text (works even if DOM structure shifts)
  // Looks for the requested day header, then extracts time, class, instructor, and status.
  if (!text) return [];

  // Build header variants for the target date
  ...
  const tabShort    = `${monShort} ${day}`; // e.g., "Sep 3"

  const lines = text.split(/\r?\n/).map(s => s.trim()).filter(Boolean);
  const norm = (s) => s.replace(/\s+/g, ' ').trim();

  // Find the start line for the target day
  let startIdx = lines.findIndex(l => {
    const t = norm(l);
    return t.includes(headerUpper) || t.includes(headerCap) || t.includes(tabShort);
  });
  if (startIdx < 0) startIdx = 0; // fallback to whole doc

  // Find the next day header to stop
  ...
  function pushCur() {
    ...
    if (cur.time && (cur.class_name || cur.status !== 'unknown')) {
      rows.push({ date: iso, time: cur.time, class_name: cur.class_name || '', instructor: cur.instructor || '', status: cur.status, open_spots: cur.open_spots });
    }
    cur = null;
  }
  ...
  for (let i = 0; i < section.length; i++) {
    ...
    // Time starts a new record
    const tm = t.match(TIME_RE);
    if (tm) {
      pushCur();
      cur = { time: `${tm[1]} ${tm[2].toUpperCase()}`, class_name: '', instructor: '', status: 'unknown', open_spots: null };
      continue;
    }
    if (!cur) continue;
    ...
    // Instructor line like "w/ Ana Maria"
    ...
    // Class name candidate (avoid time/known labels)
    if (!cur.class_name && t.length >= 3 && !TIME_RE.test(t) && !DROPIN_RE.test(t)) {
      if (!IGNORE.has(t) && !/^TIME\s+CLASS$/i.test(t)) {
        cur.class_name = t;
      }
    }
  }
...
// ---------- Text + structured rows (DOM + plain-text fallback) ----------
app.get('/capture-schedule-text', async (req, res) => {
  ...
  try {
    const result = await withBrowser(async ({ page }) => {
      ...
      // Force frame to start/end (inside the frame)
      try {
      ...
      // Try switching to Daily + the tab for the start date
      ...
      let parsed = await frame.evaluate((iso) => {
        ...
        if (candidates.length === 0) for (const sect of dayChunks) candidates.push(sect);
        ...
        for (const row of candidates) {
          ...
          const badges = Array.from(row.querySelectorAll('span, div, button')).map(e => txt(e));
          const SPOTS_RE2 = /\b(\d+)\s*SPOTS?\b/i;
          const spotsLabel = badges.find(t => SPOTS_RE2.test(t));
          ...
          if (/waitlist/i.test(btnText) || /waitlist/i.test(rowText)) {
            status = 'waitlist'; if (open_spots === null) open_spots = 0;
          } else if (/sold\s*out|^full$/i.test(btnText)) {
            status = 'full'; if (open_spots === null) open_spots = 0;
          } else if (/book/i.test(btnText) && open_spots === null) {
            status = 'open'; open_spots = null;
          }

          if (time && (class_name || status !== 'unknown')) {
            out.push({ date: iso, time, class_name: class_name || '', instructor: instructor || '', status, open_spots });
          }
        }
      ...
      if (!parsed || parsed.length === 0) {
        ...
        parsed = await frame.evaluate((iso) => {
          ...
          for (const row of cands) {
            ...
            if (!class_name) {
              const parts = t.split('\n').map(s => s.trim()).filter(Boolean);
              class_name = parts.find(p => !TIME_RE.test(p) && !/^Drop-?in:/i.test(p) && p.length > 2) || '';
            }
            ...
            out.push({ date: iso, time, class_name, instructor, status, open_spots });
          }
    ...
    // Always return parse_mode + parsed (even if empty)
    return res.json({
      start: startISO,
      end: endISO,
    ...
    // Keep 200 in debug mode so jq pipelines don't explode
